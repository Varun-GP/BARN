...
class CrowdSim(gym.Env):
    ...
    def render(self, mode='human'):
        ...
        robot_color = 'lightblue'
        goal_color = 'red'
        arrow_color = 'darkgreen'
        arrow_style = patches.ArrowStyle("->", head_length=6, head_width=2)
        ...
        # add goal
        goal=mlines.Line2D([self.robot.gx], [self.robot.gy], color=goal_color, marker='*', linestyle='None', markersize=17, label='Goal')
        ax.add_artist(goal)
        artists.append(goal)
        ...
        plt.legend([robot, goal], ['Robot', 'Goal'], fontsize=14, loc='lower left')
        ...
        behavior = [9,9,4,2,2,2,16,2,2,2,9,10,7,6,5,5,4,4,16,16]
        behavior = [1,1,1,2,2,9,9,9,9,9,2,9]
        ...
        for i in range(len(self.humans)):
            ...
            plt.text(self.humans[i].px - 0.7, self.humans[i].py - 0.7, str("Aid:"+str(behavior[i])), color='purple', fontsize=12)

            plt.text(self.robot.px,self.robot.py,"*",color='lightblue',fontsize=10)
            plt.text(self.humans[i].px,self.humans[i].py,"ߋ",color='purple',fontsize=10)

        #print(ax.texts)
        def __get__item(self,index):
            return self.text.textId[index]
